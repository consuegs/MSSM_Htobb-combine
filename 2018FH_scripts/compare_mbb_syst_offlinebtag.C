...
void compare_mbb_syst_offlinebtag( int year = 2018, string channel = "FH")
{
   ...
   string outdir = Form("plots_mbb_%d%s_syst_offlinebtag", year, channel.c_str());
   ...
   std::ofstream outFile("2018FH_systematics_offlinebtag.csv");
     if (!outFile.is_open())
     {
   ...
   for (int mp = 0; mp < samples_signal_mc["FH"].size() ; mp++)
   {
      ...
      file_up      = new TFile(Form("rootfiles_signal/mssmHbb_%d_%s_%d_offline-btag_1sup.root", year,channel.c_str(), samples_signal_mc["FH"][mp] ));
      file_down    = new TFile(Form("rootfiles_signal/mssmHbb_%d_%s_%d_offline-btag_1sdown.root", year,channel.c_str(), samples_signal_mc["FH"][mp] ));
      ...
      mbb_rebinned["up"] -> Draw("");
      mbb_rebinned["nominal"] -> Draw("hist same");
      ...
      mbb_legend -> AddEntry(mbb_rebinned["up"],"offl btag +1#sigma variation","lep");
      mbb_legend -> AddEntry(mbb_rebinned["down"],"offl btag -1#sigma variation","lep");
      mbb_legend -> Draw();
   ...
   // X axis ratio plot settings
      ...
      C -> SaveAs(Form("%s/offlinebtag_%d_GeV.png", outdir.c_str(), samples_signal_mc["FH"][mp])); 
      C -> SaveAs(Form("%s/offlinebtag_%d_GeV.pdf", outdir.c_str(), samples_signal_mc["FH"][mp]));
      C -> SaveAs(Form("%s/offlinebtag_%d_GeV.root", outdir.c_str(), samples_signal_mc["FH"][mp]));  
      ...
      C -> SaveAs(Form("%s/offlinebtag_%d_GeV_param.png", outdir.c_str(), samples_signal_mc["FH"][mp])); 
      C -> SaveAs(Form("%s/offlinebtag_%d_GeV_param.pdf", outdir.c_str(), samples_signal_mc["FH"][mp]));
      C -> SaveAs(Form("%s/offlinebtag_%d_GeV_param.root", outdir.c_str(), samples_signal_mc["FH"][mp]));  

...
map<int, vector < double>> mass_fit = {
    { 300,{ 312.89, 21.6052, 16.0848, 0.685596, 12,  1.02582 }},
    { 350,{ 350.587, 28.8859, 1.73164, 1.30584, 133.137, 0.917272}},
    { 400,{ 390.247, 36.4705, 1.51465, 1.48902, 138.557, 1.2438}},
    { 450,{ 432.399, 40.4998, 1.07447, 1.50553, 140.686, 1.7075}},
    { 500,{ 475.705, 46.0762, 0.959325, 1.67314, 134.472, 1.51321}},
//    { 600,{ 569.184, 50.2003, 0.657058, 1.60282, 125.829, 2.16902}},
    { 600,{ 569.184, 50.2003, 0.657058, 1.60282, 125.829, 2.16902}},
    { 700,{ 662.796, 56.5216, 0.574413, 1.72228, 111.437, 2.02098}},
    { 800,{ 759.958, 60.5574, 0.494413, 1.7853, 121.678, 2.08661}},
    { 900,{ 857.628, 62.2775, 0.43336, 1.79396, 111.395, 2.0203}},
    { 1000,{ 951.606, 69.0447, 0.430504, 1.95674, 39.1388, 1.61016}},
    { 1200,{ 1144.7, 75.9069, 0.394323, 1.88788, 9.85929, 2.3753}},
    { 1400,{ 1338.8, 82.4682, 0.326254, 2.02182, 12.426, 2.35765}},
    { 1600,{ 1519.11, 96.7544, 0.333443, 2.15434, 8.41591, 1.84743 }},
    { 1800,{ 1703.6, 104.172, 0.279666, 2.85086, 16.2599, 77.2316}},
};
	...
	TString SysVariations)
{
	...
	for (auto histName: histNames)
	{
		...
		outtext << std::endl;
		outtext << std::endl;
		outtext << "Histogram name : " << SysVariations << std::endl;
		outtext << std::endl;
		...
		double norm = hist->GetSumOfWeights() / lumi_sf[mass] * l1prefiring_sf[mass];
		...
		RooDoubleCB cbx("cb", "CBshape", mbb, meanx, sigmax, alpha1x, n1x, alpha2x, n2x);

		RooDataHist sig("sig", "sig", mbb, hist);
		w->import(sig);

		if (SysVariations == "_JER_1sup" || SysVariations == "_JER_1sdown")
		{
		...
		RooPlot *frame1 = mbb.frame(Name("MMSM H->bb analysis"), Title("   "));
		...
		latex.DrawLatexNDC(xpad1Leg - pad1->GetRightMargin(), 0.98 - pad1->GetTopMargin(),
			(std::string("M_{12} = ").c_str() + Mass + std::string(" GeV").c_str()));
		...
		c1->Print("Figs/Mass" + Mass + "_SR" + SysVariations + "_" + histName + "_doubleCB.png");
		c1->Print("Figs/Mass" + Mass + "_SR" + SysVariations + "_" + histName + "_doubleCB.pdf");
		delete c1;
...
void CreateSignalTemplates_FH_UpDown()
{
	...
	TString SysVariations[4] = { "_JER_1sup", "_JER_1sdown", "_JES_1sup", "_JES_1sdown"};
	int nVariations = 4;
	...
	for (unsigned int j = 0; j < nVariations; ++j)
	{
		...
		for (unsigned int i = 0; i < masses.size(); ++i)
		{
			...
			for (auto histName: histNames)
			{
				TFile *file = new TFile(dir + "/mssmHbb_2018_FH_" + Mass + SysVariations[j] + ".root", "READ");

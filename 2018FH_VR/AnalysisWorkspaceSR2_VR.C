...
int AnalysisWorkspaceSR2_VR()
{
	...
	TString Tsrmasses[5] = { "400", "450", "500", "600"};	//SR2
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		double TF_pol2_quad_centralValue =  3.6567e-07 ; //MC
		double TF_pol2_linear_centralValue = 8.17457e-05;
		...
		//wspace.import(RDHTF); //normalized TF shape from data
		wspace.import(RDHCR);
		wspace.import(RDHSR);

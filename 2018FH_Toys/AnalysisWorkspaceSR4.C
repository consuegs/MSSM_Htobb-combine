...
int AnalysisWorkspaceSR4()
{
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		TFile *f_signal_in_unbinned = new TFile("/nfs/dust/cms/user/leyvaped/Analyses/MSSM/FullRun2/Combine/April_2023/CMSSW_11_3_4/src/Analysis/Combine/Run2018/input_doubleCB_Feb23/signal_m" + Tsrmasses[mass] + "_SR4.root", "READ");
		...
		//quadratic TF
		/*double TF_pol2_quad_centralValue = 2.96322e-07; //qcd mc quadratic function
		double TF_pol2_linear_centralValue = 0.00107813; //qcd mc quadratic function
		...
		RooGenericPdf TF("TF", "TF", "TF_pol2_quad*(mbb-700)*(mbb-700)+TF_pol2_linear*(mbb-700)+1", varsTF);	// 
		cout << "TF_pol2_quad       = " << TF_pol2_quad.getVal() << endl;
		cout << "TF_pol2_linear     = " << TF_pol2_linear.getVal() << endl;*/

		//linearTF		
		double TF_pol1_linear_centralValue =  0.00116578; //qcd mc linear function
		...
		RooGenericPdf TF("TF", "TF", "TF_pol1_linear*(mbb-700)+1", varsTF);	// 

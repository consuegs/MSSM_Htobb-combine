...
int AnalysisWorkspaceSR1()
{
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		//quadratic TF
		/*double TF_pol2_quad_centralValue = 2.03653e-06; //qcd mc quadratic function
		double TF_pol2_linear_centralValue = 0.00159427; //qcd mc quadratic function
		...
		RooGenericPdf TF("TF", "TF", "TF_pol2_quad*(mbb-420)*(mbb-420)+TF_pol2_linear*(mbb-420)+1", varsTF);	// 
		cout << "TF_pol2_quad       = " << TF_pol2_quad.getVal() << endl;
		cout << "TF_pol2_linear     = " << TF_pol2_linear.getVal() << endl;*/

		
		//linearTF		
		double TF_pol1_linear_centralValue =  0.00158831; //qcd mc linear function
		...
		RooGenericPdf TF("TF", "TF", "TF_pol1_linear*(mbb-420)+1", varsTF);	// 
